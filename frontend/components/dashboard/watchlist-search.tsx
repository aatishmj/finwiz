...
export function WatchlistSearch({ onStockAdded }: { onStockAdded?: () => void }) {
  ...
  const addToWatchlist = async (stock: (typeof stocksData)[0]) => {
    ...
    // Check if stock is an Indian stock (in stocksData)
    const isIndianStock = stocksData.some(s => s.symbol === stock.symbol)
    if (!isIndianStock) {
      toast({
        title: "Invalid Stock",
        description: "Only Indian stocks can be added to the watchlist.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("Calling onStockAdded callback")
onStockAdded?.()
      } else {
        toast({
          title: "Already in Watchlist",
